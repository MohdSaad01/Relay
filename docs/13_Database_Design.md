...
**Reasoning:**
- `device_identifier` is a UUID generated by the Android app itself on first install and presented during pairing. It is the stable external identity; the primary key stays a plain internal integer. (P43: prior to this milestone the Android app actually generated a fresh UUID on every pairing *attempt* rather than once per install, contrary to this contract — fixed in `android/src/pairing/deviceIdentifier.ts`, which now persists it in the app's own private storage the first time it's needed and reuses it afterward. It is still lost on app uninstall, same as before — that part of "stable" was always scoped to the life of one install, not across a reinstall.)
- `device_secret_hash` is a long-lived credential established at pairing time, used to mint new `sessions` rows after the app restarts without re-pairing. Only the hash is stored, never the plaintext secret. **Immutable, with one exception (P43):** if a pairing request presents a `device_identifier` that already belongs to a row in this table — a re-pair of a device this desktop already trusts, not a brand new one — `PairingService.approve_pairing` reconciles onto that existing row instead of creating a duplicate, which does rotate `device_secret_hash` (a fresh secret for the fresh handshake) and deletes every session previously issued to that device. `device_name`, `platform`, and `paired_at` are left untouched by a reconciliation, so an existing rename survives it. See `backend/README.md`'s Pairing API section and `docs/15_QA_NOTEBOOK.md`'s P43 entry.
- **Device removal is a hard delete** — approved for Version 1. Removing a device deletes its row outright rather than flagging it inactive. This is simpler than a soft-delete/status model and satisfies the requirement that session tokens be invalidated on removal for free, via `ON DELETE CASCADE` on `sessions`. Soft deletion may be reconsidered in a future version if a "recently removed devices" feature is ever needed. Removal remains the only way to prune a `Device` row whose Android install was genuinely uninstalled (P43): the backend has no way to detect an Android uninstall, so such a row is not deleted or flagged automatically — it stays "Paired" until the desktop user explicitly removes it, or reconciles automatically if that same install ever re-pairs (impossible after a genuine uninstall, since the identifier does not survive one).
...
- **`id` is not a durable external identity (Milestone P17).** "Autoincrement" above means ordinary SQLite `INTEGER PRIMARY KEY` rowid assignment, not the `AUTOINCREMENT` keyword (this table's model does not use it, confirmed by the absence of a `sqlite_sequence` row for it) — once every `shared_folders` row is deleted, the next insert restarts numbering from 1, so a deleted folder's id can be reused by an unrelated one shared later. This is normal SQLite behavior, not a bug, and mirrors §1's own `devices.device_identifier` precedent: the primary key is an internal implementation detail. A client that must recognize "is this still the same logical folder I saw before" needs a signal beyond the bare id — `shared_at` (immutable, set once at row creation, untouched by an in-place re-share of the same still-live path) serves that role for Android's own local identity registry; see `CLAUDE.md`'s "Backend ID Reuse (P17)" note.
